...
class StorageAgent:
    def __init__(self, num_workers):
        self.q_table = {}
        self.visit = {}
        self.chosen_accuracy = 0.
        self.num_workers = num_workers
        self.LocalRandGen = np.random.RandomState()
        # Choose a random process to update global Q-table and visit count
        self.update_process = self.LocalRandGen.randint(0, num_workers) + 2
        self.buffer = deque()

    def update_tables(self, q_table, visit, buffer, valid_accuracy, process):
        # 1/num_workers chance of updating the global Q-table and visit count
        if process == self.update_process:
            self.chosen_accuracy = valid_accuracy
    ...
    def get_tables(self):
        self.update_process = self.LocalRandGen.randint(0, self.num_workers) + 2
        return self.q_table, self.visit, self.buffer, self.chosen_accuracy
...
class WorkerAgent:
    def __init__(self, barrier, update_freq=5000, pretrain=50000):
        # Multiprocessing
    ...
    def train(self, global_storage):
        ...
        while valid_acc < 0.7: 
            ...
            if CUR_EPISODE % self.update_freq == 0 and CUR_EPISODE >= self.pretrain:
                ...
                global_storage.update_tables(self.q_table, self.visit, self.replay_buffer.get_buffer(), \
                                             self.valid()[0], int(current_process().name.split('-')[1]))
